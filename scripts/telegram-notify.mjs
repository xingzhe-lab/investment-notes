...
const publishAll = existsSync("content/.telegram-publish-all")
const changed = publishAll
  ? listMarkdownFiles("content")
...
console.log(`准备发布 ${articles.length} 篇${publishAll ? "首次批量" : "更新"}文章。`)
...
function listMarkdownFiles(directory) {
  return readdirSync(directory, { withFileTypes: true })
    .flatMap((entry) => {
      ...
      return entry.isFile() && entry.name.endsWith(".md") && !entry.name.endsWith(".gitkeep") ? [file] : []
    })
